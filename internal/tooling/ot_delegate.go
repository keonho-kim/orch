...
func (r *OTRunner) RunDelegateTask(
	...
	request, err := buildDelegateExecRequest(workspaceRoot, record, task)
	if err != nil {
		return "", err
	}
	if wait {
	...
	return runAsyncDelegate(ctx, nextEnv, request, task)
}

func buildDelegateExecRequest(workspaceRoot string, record domain.RunRecord, task domain.SubagentTask) (domain.ExecRequest, error) {
	...
	if err != nil {
		return domain.ExecRequest{}, fmt.Errorf("marshal subagent task: %w", err)
	}
	return domain.ExecRequest{
		...
		Args: []string{
			"__subagent-run",
			repoRoot,
			placeholderRunValue(record.SessionID),
			placeholderRunValue(record.RunID),
			string(encodedTask),
		},
...
func placeholderRunValue(value string) string {
...
func runAsyncDelegate(ctx context.Context, nextEnv []string, request domain.ExecRequest, task domain.SubagentTask) (string, error) {
	...
	command := exec.Command(request.Command, request.Args[:4]...)
	command.Args = append(command.Args, string(encodedTask))
	...
	return waitForAsyncDelegateStart(ctx, command, startPath, waitDone, ticker, timeout)
}

func waitForAsyncDelegateStart(
	ctx context.Context,
	command *exec.Cmd,
	startPath string,
	waitDone <-chan error,

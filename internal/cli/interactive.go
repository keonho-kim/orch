...
func runTUI(repoRoot string, configFile string, restoreSessionID string, showHistory bool, restoreLatest bool) error {
	app, err := newApp(repoRoot, configFile, orchestrator.BootOptions{RestoreSessionID: restoreSessionID})
	if err != nil {
		return err
	}
	defer app.close()
